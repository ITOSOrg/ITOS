	...
	public String updateCodeTableHeader(CodeTableHeaderDetail codeTableHeaderDetail) {
		...
		if (CodeTableHeaderVersionNoFromUpdate == CodeTableHeaderVersionNoFromDatabase) {
			...
			try {
				...
				String codeTableHeaderSQLStr = "UPDATE	CodeTableHeader	SET tableName = ?, defaultCode = ?, recordStatus = ?, versionNo = ?, createdBy = ?, lastModifiedBy = ? "
						+ "WHERE tableName = '" + codeTableHeaderDetail.getTableName() + "'";
